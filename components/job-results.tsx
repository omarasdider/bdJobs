...
export default  async function  JobResults({filterValues, 
  page = 1,
   ...
   const skip = (page -1) * jobPerPage
    ...
    const jobsPromise = await db.job.findMany({
        where,
   ...
   {jobs.map((job) => (
    ...
    {jobs.length === 0  && (
      <p className="m-auto text-center"> No jobs found adjusting your search filters</p>
    )}
    {jobs.length > 0 && (
      <Pagination currentPage={page}
      totalPages={Math.ceil(totalResult / jobPerPage)}
